...
# Enable TF32 for H100 optimization using old API
# Note: Using only old API to avoid conflicts with PyTorch 2.9+ internals
...
# -----------------------------------------------------------------------------
# NorMuon optimizer (simplified for single GPU)

class NorMuon(torch.optim.Optimizer):
    """
    Muon - MomentUm Orthogonalized by Newton-schulz (Simplified for single GPU)

    https://kellerjordan.github.io/posts/muon/

    Muon internally runs standard SGD-momentum, and then performs an orthogonalization post-
    processing step, in which each 2D parameter's update is replaced with the nearest orthogonal
    matrix. To efficiently orthogonalize each update, we use Polar Express.

    Warning: This optimizer should not be used for the embedding layer, the final fully connected layer,
    or any {0,1}-D parameters; those should all be optimized by a standard method (e.g., AdamW).
    Though empirically small 1D params perform efficiently here:
        Polar Express approximately performs a magnitude normalization of the grad
        This hyper-optimized class has faster execution time than the current impl of Adam for small params
    """
    def __init__(self, params, lr=0.02, weight_decay=0.01, momentum=0.95, beta2=0.95):
        defaults = dict(lr=lr, weight_decay=weight_decay, momentum=momentum, beta2=beta2)
        param_groups = self.generate_param_groups(params)
        super().__init__(param_groups, defaults)

    def generate_param_groups(self, params):
        """
        Create one param group per size, while giving attn its own param group for reshape op.
        """
        params = list(params)
        param_groups = []
        attn_subset = [p for p in params if hasattr(p, 'label') and p.label == 'attn']
        non_attn_subset = [p for p in params if not (hasattr(p, 'label') and p.label == 'attn')]
    ...
    @torch.no_grad()
    def step(self):
        # Simplified single-GPU implementation
        for group in self.param_groups:
            ...
            # Collect gradients for batched processing
            grads_list = []
            for param in params:
                ...
                # Compute the actual update_grad for polar_express
                update_grad = grad.lerp(momentum_buffer, group["momentum"])
                grads_list.append(update_grad)

            # Stack for batched polar_express computation
            batched_update_grads = torch.stack(grads_list)

            # Reshape attn params from [hdim, dim*4] to [4,hdim,dim] to apply polar_express independently to Q,K,V,O
            original_shape = batched_update_grads.shape
            if params and hasattr(params[0], 'label') and params[0].label == 'attn':
            ...
            # Initialize second momentum buffer on first iteration
            if "second_momentum_buffer" not in self.state[params[0]]:
                if params[0].size(-2) >= params[0].size(-1):
                    self.state[params[0]]["second_momentum_buffer"] = torch.zeros(
                        (len(params), *params[0].shape[:-1], 1),
                        dtype=torch.float32,
                        device=params[0].device
                    )
                else:
                    self.state[params[0]]["second_momentum_buffer"] = torch.zeros(
                        (len(params), *params[0].shape[:-3], 1, params[0].shape[-1]),
                        dtype=torch.float32,
                        device=params[0].device
                    )

            second_momentum_buffer = self.state[params[0]]["second_momentum_buffer"]

            # Compute adaptive step size using second moment
            ...
            for i, param in enumerate(params):
                ...
                # Apply the update
                param.add_(v_chunk[i], alpha=-eff_lr_val)


class SingleGPUAdam(torch.optim.Optimizer):
    """
    Standard Adam optimizer optimized for single GPU usage.
    Simplified from DistAdam for single-device operation.
    """
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, head_dim: int, model_dim: int, max_seq_len: int,
        ...
        # Single GPU: no padding needed for distributed
...
# -----------------------------------------------------------------------------
# Data loader (simplified for single GPU)
...
def single_gpu_data_generator(filename_pattern: str, num_tokens: int, max_seq_len: int, grad_accum_steps: int = 1, align_to_bos: bool = True):
    """
    Simplified data generator for single GPU training.
    ...
    while True:
        max_num_docs = next_multiple_of_n(num_tokens // 300, n=128)  # median doc length is ~400

        if align_to_bos:
            try:
                seq_starts, seq_ends = finder.next_batch(num_tokens, max_seq_len)
                start_idxs, end_idxs = torch.tensor(seq_starts), torch.tensor(seq_ends)
            ...
            _inputs = buf[:-1].view(num_tokens, )
            _targets = buf[1:].view(num_tokens, )
...
rank = 0
world_size = 1
# Gradient accumulation is beneficial for single GPU!
...
train_loader = single_gpu_data_generator(args.train_files, args.train_batch_size, args.train_max_seq_len, grad_accum_steps=grad_accum_steps)
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        val_loader = single_gpu_data_generator(args.val_files, args.val_batch_size, -1, grad_accum_steps=grad_accum_steps, align_to_bos=False)
        val_loss = 0
        with torch.no_grad():
